...
FROM (
    SELECT
        ROW_NUMBER() OVER(PARTITION BY OrderID ORDER BY CreationTime) AS Dup,
        *
    FROM Sales.OrdersArchive
) t
WHERE Dup = 1;
...
SELECT
    ...
    NTILE(2) OVER(ORDER BY Sales) AS TwoBucketsByProd,
    NTILE(1) OVER(ORDER BY Sales) AS OneBuckets
FROM Sales.Orders;
